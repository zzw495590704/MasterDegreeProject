...
uint8_t write_temp_bin(uint8_t* data, std::string path) {
	// 打开一个二进制文件以写入模式
	...
	// 写入数据到二进制文件
	size_t written = fwrite(data, sizeof(uint8_t), 98304, outFile);
	if (written != 98304) {
		perror("写入数据失败");
		fclose(outFile); // 确保文件关闭
		return 1;
	}
...
void usr_test_func(void* frame, void* usr_param)
{
    ...
    if (stream_frame_info->raw_frame != NULL)
    {
		...
		std::string filename = "../data/frame_" + std::to_string(frame_idx) + "_" + std::to_string(1) + ".bin";
		//std::string filename = "../data.bin";
		write_temp_bin((uint8_t*)stream_frame_info->temp_frame, filename);
